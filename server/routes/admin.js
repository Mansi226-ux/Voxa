...
router.get("/stats", adminAuth, async (req, res) => {
  try {
    ...
    // Get recent activity
    const recentPosts = await Post.find().populate("authorId", "name avatar").sort({ createdAt: -1 }).limit(5)
...
router.get("/posts", adminAuth, async (req, res) => {
  try {
    ...
    const posts = await Post.find().populate("authorId", "name avatar").sort({ createdAt: -1 }).skip(skip).limit(limit)
