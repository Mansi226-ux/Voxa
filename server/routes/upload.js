...
const { upload, profileUpload, deleteImage, extractPublicId } = require("../config/cloudinary.js")
...
router.post("/profile-image", auth, profileUpload.single("avatar"), async (req, res) => {
  try {
    if (!req.file) {
      return res.status(400).json({ message: "No image file provided" })
    }
